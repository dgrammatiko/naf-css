...
module.exports.compile = async (file, flags) => {
  ...
  if (flags.nonMinified) {
    ...
    process.stdout.write(kleur.green(`File ${file.replace('src', 'css')}${flags.legacy ? '-ie' : ''}.css [Brotli-size=${brotliSize.sync(css)}, Uncompressed-size=${css.length}] was created succesfully 👍 `) + "\n");
  }
  ...
  if (flags.minified) {
    ...
    process.stdout.write(kleur.green(`File ${file.replace('src', 'css').replace('.css', '')}${flags.legacy ? '-ie' : ''}.min.css [Brotli-size=${brotliSize.sync(min.css)}, Uncompressed-size=${min.css.length}] was created succesfully 👍 `) + "\n");
  }
}
